...
export default function Abilities() {
    ...
    const { data: fetchedTranslations } = useQuery({
        ...
        queryFn: () => apiClient.get<AbilityTranslation[]>(ENDPOINTS.abilities.translations(modalId!)),
    ...
    const saveSymbol = useMutation({
        mutationFn: ({ id, symbol }: { id: number; symbol: string }) =>
            apiClient.patch(ENDPOINTS.abilities.symbol(id), { symbol }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['abilities'] });
            queryClient.invalidateQueries({ queryKey: ['ability-count'] });
            addToast('Symbol saved', 'success');
        },
        onError: () => addToast('Failed to save symbol', 'error'),
    });

    const addDefault = useMutation({
        mutationFn: () => apiClient.post<Ability>(ENDPOINTS.abilities.default),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['abilities'] });
            queryClient.invalidateQueries({ queryKey: ['ability-count'] });
            addToast('Ability added', 'success');
        },
        onError: () => addToast('Failed to add ability', 'error'),
    });

    const deleteAbility = useMutation({
        mutationFn: (id: number) => apiClient.delete(ENDPOINTS.abilities.byId(id)),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['abilities'] });
            queryClient.invalidateQueries({ queryKey: ['ability-count'] });
            addToast('Ability deleted', 'success');
        },
        onError: () => addToast('Failed to delete ability', 'error'),
    });
    ...
    const saveTranslations = useMutation({
        mutationFn: async () => {
            ...
            await Promise.all(
                LANGUAGES.map((lang) =>
                    apiClient.put(ENDPOINTS.abilities.translation(modalId, lang), translations[lang])
                )
            );
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['ability-translations', modalId] });
    ...
    return (
        <>
                ...
                <table className="global-content-table">
                    ...
                    <tbody>
                        {abilities.map((ability) => (
                            <tr key={ability.id} className="table-tr-base-height">
                                <td>{ability.id}</td>
                                <td>
                                    <input
                                        className={`global-text-input ${styles.symbolInput}`}
                                        type="text"
                                        defaultValue={ability.symbol}
                                        onBlur={(e) => {
                                            if (e.target.value !== ability.symbol) {
                                                saveSymbol.mutate({ id: ability.id, symbol: e.target.value });
                                            }
                                        }}
                                    />
                                </td>
                                <td>
                                    <button onClick={() => setModalId(ability.id)}>{LANG_ICON}</button>
                                </td>
                                <td>
                                    <button onClick={() => deleteAbility.mutate(ability.id)}>{DELETE_ICON}</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            {modalId != null && (
                <LangModal
                    title={`Edit translations — ${abilities.find((a) => a.id === modalId)?.symbol ?? ''}`}
